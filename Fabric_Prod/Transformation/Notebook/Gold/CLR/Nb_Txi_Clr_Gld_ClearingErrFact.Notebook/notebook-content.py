...
# Load Staging Table
df_staging = spark.read.format("delta").table("SilverLakehouse.ClearingErrFact")

# # Load Data Warehouse Table
df_dwh = spark.read.format("delta").table("GoldLakehouse.ClearingErrFact")
...
df_new_records = df_staging.alias("staging").join(
    df_dwh.alias("dwh"),
    col("staging.ClearingErrCode") == col("dwh.ClearingErrCode"),
...
df_new_records.write.format("delta").mode("Append").saveAsTable("GoldLakehouse.ClearingErrFact")
